...
const _setDebugEvents = () => {
  ...
  eventsPage.forEach((event) =>
    page.on(event, () => console.log("page event " + event + " raised"))
  );
  eventsContext = ["close", "page"];
  eventsPage.forEach((event) =>
    context.on(event, () => console.log("context event " + event + " raised"))
  );
};
...
module.exports.openBrowser = async (options = {}) => {
  ...
  if (browser) throw new Error(`Browser already opened`);

  browser =
    options.browserType &&
    typeof options.browserType == "string" &&
    (options.browserType == "chromium" ||
      options.browserType == "firefox" ||
      options.browserType == "webkit")
      ? browsers[options.browserType]
      : defaultConfig.browser;

  if (!browser)
    throw new Error(
      `Unknown browser, Please set one of the given browser\nchromium\nfirefox\nwebkit`
    );

  if (options.observe) options.slowMo = options.observeTime;
  ...
  if (options.url) return await this.openContext(options);
};
...
module.exports.openContext = async (options = {}) => {
  if (!browser) throw new Error(`Browser not opened`);
  ...
  if (options.url) return await this.openPage(options);
};
...
module.exports.openPage = async (options = {}) => {
  ...
  if (!(browser && context))
    throw new Error(
      "Context not initialized. Call `openContext()` before using this API"
    );
  page = await context.newPage(options);
  page.name = options.pageName || "defaultPage-" + Date.now();
  if (options.setDebugEvents) _setDebugEvents();

  if (options.url) await this.goto(new URL(options.url).toString(), options);
  return page;
};
...
module.exports.goto = async (url, options = {}) => {
  options = { ...userOptions, ...options };
  validate();
  if (options.extraHTTPHeaders) await page.setExtraHTTPHeaders(headers);

  let optionsWithoutUrl = options;
  delete optionsWithoutUrl.url;
  await _navigationFunction(
    page.goto(url, optionsWithoutUrl),
    optionsWithoutUrl
  );
};
...
module.exports.switchPage = async (arg) => {
  ...
  await Promise.all(
    pages.map(async (page) => {
      page.currentTitle = await page.title();
      return page;
    })
  );
  ...
  pages = Object.prototype.toString.call(arg).includes("RegExp")
    ? pages.filter(
        (page) =>
          page.url().match(new RegExp(arg)) ||
          page.currentTitle.match(new RegExp(arg)) ||
          page.pageName.match(new RegExp(arg))
      )
    : pages.filter(
        (page) =>
          page.url() === arg ||
          page.currentTitle === arg ||
          page.pageName === arg
      );
  ...
  if (pages.length > 0) {
    ...
    throw new Error(`Cannot find title or URL matching or pageName ` + arg);
  }
};
 ...
 * # switch using context name
 * await switchContext('ThirdContext')
 *
 * @param {string} arg - URL/Page title of the tab to switch.
...
module.exports.switchContext = async (arg, name) => {
  ...
  if (name) {
    let filteredContexts = contexts.filter((context) => context.name === name);
    if (filteredContexts.length != 1)
      throw new Error("Found " + filteredContexts.length + " matching " + name);
    context = filteredContexts[0];
    return await this.switchPage(arg);
  }

  contexts = contexts.filter((context) => context.name === arg);
  if (contexts.length == 1 && contexts[0].pages().length > 0) {
    context = contexts[0];
    page = context.pages()[0];
  } else {
    throw new Error(`Cannot find contextName matching` + arg);
  }
...
module.exports.closeContext = async (name) => {
  ...
  browserHistory = browserHistory.filter(
    (acp) => acp.context.name !== contextToClose
  );
...
module.exports.closePage = async (arg, contextName) => {
  ...
  if (
    ...
    pages.forEach(async (targetPage) => {
      browserHistory = browserHistory.filter(
        (acp) =>
          acp.context.name !== targetPage.context &&
          acp.page.uuid !== targetPage.uuid
      );
      if (targetPage == page) currentPageClosed = true;
...
module.exports.spy = async (requestUrl, onEvent, option) => {
  if (
    ...
    !Object.prototype.toString.call(requestUrl).includes("RegExp")
  ) {
    throw new TypeError(
      'The "requestUrl" argument must be of type string or regex. Received type ' +
        typeof arg
    );
  }

  if (!context.spies) context.spies = [];

  context.spies.push({ requestUrl: requestUrl, option: option });
  page.on(onEvent, (request) => {
    let [matchRequest] = context.spies.filter((r) =>
      request.url().match(r.requestUrl)
    );
    if (matchRequest) matchRequest.option(...[request]);
  });
};
...
module.exports.intercept = async (requestUrl, option, count) => {
  if (!context.interceptedRequests) context.interceptedRequests = [];
  ...
  await context.route(new RegExp(requestUrl), (request) => {
    ...
    [matchRequest] = context.interceptedRequests.filter((r) =>
      request.request().url().match(r.requestUrl)
    );

    if (matchRequest) {
      if (!matchRequest.option) request.abort();
    ...
    context.interceptedRequests = context.interceptedRequests.filter(
      (r) => r.count != 0
    );
  });
};
...
/**
 * Pause the test running
 *
 * For the useage see [page pause](https://playwright.dev/docs/api/class-page#page-pause)
 * @example
 * await pause()
 *
 * @returns void
...
module.exports.deleteCookies = async (name) => {
  ...
  if (name) {
    ...
    let filteredCookies = cookies.filter(
      (cookie) =>
        JSON.stringify(cookie) !== JSON.stringify({ ...cookie, ...name })
    );
...
_navigationFunction = async (func, options = {}) => {
  ...
  if (
    (options.waitForEvent && !Array.isArray(options.waitForEvent)) ||
  ...
  if (options.waitForRequest)
    options.waitForRequest.forEach((e) =>
      promises.push(page.waitForRequest(e))
    );
  if (options.waitForResponse)
    options.waitForResponse.forEach((e) =>
      promises.push(page.waitForResponse(e))
    );
  if (options.waitForNavigation) promises.push(page.waitForNavigation());
 ...
 * Fetches an element with the given selector, scrolls it into view if needed, and then clicks in the center of the element. If there's no element matching selector, the method throws an error.
 * @example
 * await dragAndDrop($('[id = source]'), $('[id = target]'))
 *
 * @param {selector} source - A selector to search for element is source.
 * @param {selector} target - A selector to search for element is target.
 * @param {Object} options see [Detailed Options](https://playwright.dev/docs/api/class-page#page-drag-and-drop)
 *
 * @returns {Promise}
 */
module.exports.dragAndDrop = async (source, target, options = {}) => {
  options = { ...userOptions, ...options };
  validate();

  let elementHandle = await selector.getElementHandle();
  await _navigationFunction(elementHandle.dragAndDrop(source, target, options), options);
};
...
/**
 * Select an options from the given dropdown
 *
 * @example
 * await focus($('text=Username:'))
 *
 * @param {selector|string} selector - A selector of an dropdown element. If there are multiple elements satisfying the selector, the first will be focused.
 * @param {string|Array} selectOptions - An option or array of aptions to select from the given dropdown.
 */
 module.exports.dropDown = async (selector, selectOptions) => {
  validate();
  let elementHandle = await selector.getElementHandle();
...
/**
 *  the given text into the focused or given element.
 * @example
...
/**
 * Fill the given text into the focused or given element.
 * @example
 * await type('admin', $('[id=text]'))
 * await write('admin')
 *
 * @param {string} text - Text to type into the element.
 * @param {Object} options
 ...
 */
 ...
 module.exports.type = async (text, selector, options) => {
  let elementHandle = await selector.getElementHandle();
  await _navigationFunction(elementHandle.type(text, options), options);
};
...
module.exports.press = async (keys, options) => {
  !Array.isArray(keys)
    ...
    : keys.forEach(
        async (key) =>
          await _navigationFunction(page.keyboard.press(key, options), options)
      );
};
...
module.exports.mouseAction = async (
  action,
  selector,
  coordinates,
  options = {}
  ...
  if (targetAction) {
    ...
    throw new Error(
      `Unknown action, Please set one of the given press\nmove\nrelease`
    );
};
...
module.exports.screenshot = async (selector, options = {}) => {
  validate();
  let elementHandle =
    selector && selector instanceof Selector
      ? await selector.getElementHandle()
      : page;

  options = { ...userOptions, ...options };
  options.path = options.path || `Screenshot-${Date.now()}.png`;
  let screenShot = await elementHandle.screenshot(options);
  if (options.encoding)
...
function Selector(selector, options) {
  ...
  typeof selector == "string" ? (this.selector = selector) : (this.elementHandle = selector);

  this.getElementHandle = async (options) => {
    return this.selector ? await this._updateElementHandle(options) : this.elementHandle;
  };

  this.exists = async (options = {}) => {
    options = { ...options, implicitWait: false };
    return (await this.getElementHandle(options)) != undefined;
  };

  this.getAttribute = async (name, options = {}) => {
    let element = await this.getElementHandle(options);
    return await element.getAttribute(name);
  };

  this.text = async () => {
    let element = await this.getElementHandle();
      ...
      try {
        ...
        if (options.implicitWait) {
          return await Promise.race(
            frames.map(
              async (frame) => await frame.waitForSelector(selector, options)
            )
          );
        } else {
          let elementHandles = await Promise.all(
            frames.map(async (frame) => await frame.$(selector))
          );
          [elementHandles] = elementHandles.filter(
  ...
  this.elements = async (options = {}) => {
    ...
    do {
      ...
      try {
        ...
        elementHandles
          .filter((elementHandle) => elementHandle.length != 0)
          .flat()
          .forEach((e) => {
            ret.push(new Selector(e, options));
          });
        return ret;
      } catch (e) {}
    } while (true);
...
module.exports.getConfig = async (config) => {
  if (config) return userOptions[config];
};
...
module.exports.waitFor = async (elementOrFunc, args, options = {}) => {
  ...
  if (
    (typeof elementOrFunc == "string" && !isNaN(parseInt(elementOrFunc))) || typeof elementOrFunc == "number"
  ) {
    let time =
      typeof elementOrFunc == "string"
        ? parseInt(elementOrFunc)
        : elementOrFunc;
    return await page.waitForTimeout(time);
  }
  if (elementOrFunc instanceof Selector) {
    options.state = args || "visible";
    return await elementOrFunc.getElementHandle({
      ...options,
      implicitWait: true,
    });
  }
  ...
  if (typeof elementOrFunc == "function" && Array.isArray(args)) {
    args = await Promise.all(
      args.map(async (arg) => {
        return arg instanceof Selector ? await arg.getElementHandle() : arg;
      })
    );
